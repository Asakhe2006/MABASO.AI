...
assert.doesNotMatch(appSource, /className="collaboration-mobile-room-tabs"/, "Mobile must not duplicate room controls above the room.");
...
assert.match(appSource, /saveCollaborationBoardItem/, "Existing board items must save edits without requiring a second post.");
assert.match(appSource, /renderActiveCollaborationTool/, "Room-owner generation tools must render in the materials workspace.");
assert.match(appSource, /Academic Reports/, "The collaboration material filters must include academic reports.");
assert.match(appSource, /chat-images/, "Camera uploads must become inline room-chat messages rather than library materials.");
